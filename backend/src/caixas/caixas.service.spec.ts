...
const abertura = { id: 'abertura-1', caixaId: 'caixa-1', empresaId: 'empresa-1', aberto: true };
...
describe('CaixasService', () => {
  ...
  it('cria caixa válido no tenant do usuário', async () => {
    ...
    expect(prisma.caixa.create).toHaveBeenCalledWith(expect.objectContaining({
      data: expect.objectContaining({ empresaId: 'empresa-1', nome: 'Principal', codigo: 'CX1' }),
    }));
  });
  ...
  it.each([
    ...
    prisma.caixa.create.mockRejectedValue(new Prisma.PrismaClientKnownRequestError(
      'unique', { code: 'P2002', clientVersion: '6.19.3', meta: { target } },
    ));
    await expect(service.criar({ nome: 'Principal', codigo: 'CX1' }, usuario))
  ...
  it('não converte P2002 desconhecido', async () => {
    ...
    await expect(service.criar({ nome: 'Principal', codigo: 'CX1' }, usuario)).rejects.toBe(erro);
  });
  ...
  it('abre de forma condicional, registra histórico e usa lock', async () => {
    ...
    expect(prisma.caixa.updateMany).toHaveBeenCalledWith(expect.objectContaining({
      where: expect.objectContaining({ empresaId: 'empresa-1', status: StatusCaixa.FECHADO }),
    }));
  ...
  it('rejeita segunda abertura sem criar abertura ou histórico', async () => {
    ...
    await expect(service.abrir('caixa-1', { saldoInicial: 0 }, usuario)).rejects.toThrow('já está aberto');
  ...
  it('converte conflito do índice de abertura ativa', async () => {
    prisma.aberturaCaixa.create.mockRejectedValue(new Prisma.PrismaClientKnownRequestError(
      'unique', { code: 'P2002', clientVersion: '6.19.3',
        meta: { target: 'AberturaCaixa_caixaId_aberto_key' } },
    ));
    await expect(service.abrir('caixa-1', { saldoInicial: 0 }, usuario))
  ...
  it('rollback lógico se histórico da abertura falhar', async () => {
    ...
    await expect(service.abrir('caixa-1', { saldoInicial: 0 }, usuario)).rejects.toBe(erro);
  ...
  it('entrada usa increment e registra saldos efetivos', async () => {
    ...
    prisma.caixa.findUniqueOrThrow.mockResolvedValue(caixa(StatusCaixa.ABERTO, 15));
    prisma.aberturaCaixa.findFirst.mockResolvedValue(abertura);
    await service.criarMovimentacao('caixa-1', {
      tipo: TipoMovimentacaoCaixa.ENTRADA, descricao: 'Entrada', valor: 5,
    }, usuario);
    expect(prisma.caixa.updateMany).toHaveBeenCalledWith(expect.objectContaining({
      data: { saldoAtual: { increment: 5 } },
    }));
    expect(prisma.movimentacaoCaixa.create).toHaveBeenCalledWith(expect.objectContaining({
      data: expect.objectContaining({ saldoAnterior: 10, saldoPosterior: 15 }),
    }));
  });

  it('saída usa decrement condicional e nunca permite negativo', async () => {
    prisma.caixa.findUnique.mockResolvedValue(caixa(StatusCaixa.ABERTO, 10));
    prisma.caixa.findUniqueOrThrow.mockResolvedValue(caixa(StatusCaixa.ABERTO, 4));
    prisma.aberturaCaixa.findFirst.mockResolvedValue(abertura);
    await service.criarMovimentacao('caixa-1', {
      tipo: TipoMovimentacaoCaixa.SAIDA, descricao: 'Saída', valor: 6,
    }, usuario);
    expect(prisma.caixa.updateMany).toHaveBeenCalledWith(expect.objectContaining({
  ...
  it('rejeita saída concorrente derrotada sem movimento ou histórico', async () => {
    ...
    prisma.aberturaCaixa.findFirst.mockResolvedValue(abertura);
    prisma.caixa.updateMany.mockResolvedValue({ count: 0 });
    await expect(service.criarMovimentacao('caixa-1', {
      tipo: TipoMovimentacaoCaixa.SAIDA, descricao: 'Saída', valor: 6,
  ...
  it('fecha de forma condicional com saldo consistente e histórico', async () => {
    ...
    expect(prisma.aberturaCaixa.updateMany).toHaveBeenCalledWith(expect.objectContaining({
      where: expect.objectContaining({ aberto: true, empresaId: 'empresa-1' }),
      data: expect.objectContaining({ saldoSistema: 90, saldoInformado: 89, diferenca: -1 }),
    }));
    expect(prisma.caixaHistorico.create).toHaveBeenCalled();
  });
  ...
  it('rejeita fechamento duplicado antes de efeitos', async () => {
    ...
    await expect(service.fechar('caixa-1', { saldoInformado: 0 }, usuario)).rejects.toThrow('não está aberto');
  ...
  it('rejeita perdedor concorrente do fechamento', async () => {
    ...
    await expect(service.fechar('caixa-1', { saldoInformado: 0 }, usuario)).rejects.toThrow('já foi fechada');
    expect(prisma.caixaHistorico.create).not.toHaveBeenCalled();
  });

  it('impede operação em caixa de outro tenant', async () => {
    prisma.caixa.findUnique.mockResolvedValue({ ...caixa(), empresaId: 'empresa-2' });
    await expect(service.abrir('caixa-1', { saldoInicial: 0 }, usuario))
  ...
  it('SUPER_ADMIN mantém a empresa operacional do caixa', async () => {
    prisma.caixa.findUnique.mockResolvedValue({ ...caixa(), empresaId: 'empresa-2' });
    await service.abrir('caixa-1', { saldoInicial: 0 }, { id: 'super', tipo: 'SUPER_ADMIN' });
    expect(prisma.aberturaCaixa.create).toHaveBeenCalledWith(expect.objectContaining({
