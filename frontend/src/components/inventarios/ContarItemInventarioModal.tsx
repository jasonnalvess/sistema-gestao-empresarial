...
import { contarItemInventario, InventarioEstoque, InventarioItem } from "@/services/inventarios.service";

export function ContarItemInventarioModal({ inventario, item }: { inventario: InventarioEstoque; item: InventarioItem }) {
  ...
  async function salvar() {
    ...
    if (quantidade === "" || !Number.isFinite(valor) || valor < 0) { toast.error("Informe uma quantidade válida."); return; }
    try {
      setSalvando(true);
      await contarItemInventario(inventario.id, item.id, { quantidadeContada: valor, observacao: observacao.trim() || undefined });
    ...
    } catch (error: unknown) { toast.error(obterMensagemErro(error, "Erro ao registrar contagem")); }
