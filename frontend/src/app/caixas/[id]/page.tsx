...
import {
  PERMISSAO_CAIXA_ABRIR,
  PERMISSAO_CAIXA_EDITAR,
  PERMISSAO_CAIXA_FECHAR,
  PERMISSAO_CAIXA_MOVIMENTACOES_REGISTRAR,
...
import { EditarCaixaModal } from "@/components/caixas/EditarCaixaModal";
...
export default function CaixaDetalhesPage() {
  ...
  const {
    ...
    queryKey: caixasQueryKeys.detalhe(empresaEfetivaId ?? "", id),
  ...
  const aberto = caixa.status === "ABERTO" && caixa.ativo;
    ...
    <AppLayout>
      <div className="min-w-0 space-y-6">
        <PageHeader
          ...
          actions={
            <div className="grid w-full min-w-0 grid-cols-1 gap-2 lg:flex lg:w-auto lg:flex-wrap [&>*]:w-full md:[&>*]:w-full lg:[&>*]:w-auto">
              ...
              {podeEditar ? <EditarCaixaModal caixa={caixa} /> : null}

              {aberto && podeMovimentar && (
                <>
                  <NovaMovimentacaoCaixaModal caixa={caixa} tipo="ENTRADA" />

                  <NovaMovimentacaoCaixaModal caixa={caixa} tipo="SAIDA" />
                </>
              )}

              {aberto && podeFechar ? <FecharCaixaModal caixa={caixa} /> : null}

              {!aberto &&
                podeAbrir &&
                caixa.ativo &&
                caixa.status === "FECHADO" && <AbrirCaixaModal caixa={caixa} />}

              {podeEditar && caixa.status !== "ABERTO" && (
                <AlterarStatusCaixaButton caixa={caixa} />
              )}
            </div>
          }
        />
        ...
        <div className="grid min-w-0 grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <ResumoCard titulo="Status" valor={formatarStatus(caixa.status)} />

          <ResumoCard
            titulo="Situação"
            valor={caixa.ativo ? "Ativo" : "Inativo"}
          />

          <ResumoCard
            titulo="Sessões registradas"
            valor={String(caixa.aberturas?.length ?? 0)}
          />
        </div>
        ...
        <CrudCard>
          ...
          <div className="grid min-w-0 grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-4">
            ...
            <Campo label="Status" valor={formatarStatus(caixa.status)} />
            ...
            <Campo label="Criado por" valor={caixa.usuarioCriacao?.nome} />

            <Campo
              label="Criado em"
              valor={formatarDataHora(caixa.createdAt)}
            />

            <Campo
              label="Atualizado em"
              valor={formatarDataHora(caixa.updatedAt)}
            />
          </div>
        </CrudCard>
        ...
        <CrudCard>
          ...
          <div className="min-w-0 max-w-full overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {(caixa.movimentacoes ?? []).map((movimentacao) => (
                  <tr key={movimentacao.id} className="border-b last:border-0">
                    ...
                    <td className="p-3">
                      <TipoMovimentacaoBadge tipo={movimentacao.tipo} />
                    </td>
                ...
                {(caixa.movimentacoes?.length ?? 0) === 0 && (
                  <tr>
            ...
            {(caixa.aberturas ?? []).map((abertura) => (
              ...
              >
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-medium text-slate-900">
                        {abertura.aberto ? "Sessão aberta" : "Sessão encerrada"}
                      </p>

                      <p className="text-xs text-slate-500">
                        Abertura: {formatarDataHora(abertura.dataAbertura)}
                      </p>
                    </div>
                  </div>
                  ...
                  >
                    {abertura.aberto ? "Aberta" : "Fechada"}
                  </span>
                </div>
                ...
                <div className="mt-4 grid min-w-0 grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
                  ...
                  <Campo
                    ...
                    valor={
                      abertura.saldoSistema !== null &&
                      abertura.saldoSistema !== undefined
                        ? formatarMoeda(abertura.saldoSistema)
                        : null
                    }
                  />
                  ...
                  <Campo
                    ...
                    valor={
                      abertura.saldoInformado !== null &&
                      abertura.saldoInformado !== undefined
                        ? formatarMoeda(abertura.saldoInformado)
                        : null
                    }
                  />
                  ...
                  <Campo
                    label="Diferença"
                    valor={
                      abertura.diferenca !== null &&
                      abertura.diferenca !== undefined
                        ? formatarMoeda(abertura.diferenca)
                        : null
                    }
                  />
                  ...
                  <Campo
                    label="Aberto por"
                    valor={abertura.usuarioAbertura?.nome}
                  />

                  <Campo
                    label="Fechado por"
                    valor={abertura.usuarioFechamento?.nome}
                  />
                  ...
                  <Campo
                    ...
                    valor={
                      abertura.dataFechamento
                        ? formatarDataHora(abertura.dataFechamento)
                        : null
                    }
                  />
                  ...
                  <Campo
                    label="Movimentações"
                    valor={String(abertura._count?.movimentacoes ?? 0)}
                  />
                </div>
                ...
                {(abertura.observacaoAbertura ||
                  ...
                  <div className="mt-4 space-y-2 border-t pt-4 text-sm text-slate-600">
                    {abertura.observacaoAbertura && (
                      <p>
                        <strong>Abertura:</strong> {abertura.observacaoAbertura}
                      </p>
                    )}
...
function TipoMovimentacaoBadge({ tipo }: { tipo: TipoMovimentacaoCaixa }) {
...
function formatarStatus(status: StatusCaixa) {
  const mapa: Record<StatusCaixa, string> = {
    ABERTO: "Aberto",
    FECHADO: "Fechado",
    INATIVO: "Inativo",
  };
