...
export default function EstoquePage() {
  ...
  return <AppLayout><div className="space-y-6"><PageHeader title="Estoque" description="Acompanhe os saldos atuais dos produtos." /><CrudCard><CrudToolbar><CrudSearch value={search} onChange={setSearch} onSearch={() => { setPage(1); setSearchAplicado(search); }} placeholder="Pesquisar por produto ou código..." /></CrudToolbar>
    {error && <div className="mt-4 rounded-lg bg-red-50 p-3 text-sm text-red-700">Erro ao carregar estoque.</div>}
    {isLoading ? <CrudLoading /> : <><div className="mt-5 overflow-x-auto"><Table><TableHeader><TableRow><TableHead>Produto</TableHead><TableHead>Código</TableHead><TableHead>Atual</TableHead><TableHead>Mínimo</TableHead><TableHead>Máximo</TableHead><TableHead>Status</TableHead></TableRow></TableHeader><TableBody>
