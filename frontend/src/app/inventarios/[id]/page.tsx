...
function formatarQuantidade(valor?: string | null) {
  return valor === null || valor === undefined
    ? "-"
    : Number(valor).toLocaleString("pt-BR", { maximumFractionDigits: 4 });
}
        ...
        <PageHeader
          ...
          actions={
            <div className="grid w-full min-w-0 grid-cols-1 gap-2 lg:flex lg:w-auto lg:flex-wrap [&>*]:w-full md:[&>*]:w-full lg:[&>*]:w-auto">
              ...
              <EditarInventarioModal inventario={inventario} />
              <CancelarInventarioButton inventario={inventario} />
              <FinalizarInventarioButton inventario={inventario} />
            </div>
          }
        />
        <CrudCard>
          <div className="grid min-w-0 grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-4">
            ...
            <Campo
              label="Depósito"
              valor={`${inventario.deposito.codigo} - ${inventario.deposito.nome}`}
            />
            <Campo
              label="Abertura"
              valor={new Date(inventario.dataAbertura).toLocaleString("pt-BR")}
            />
            <Campo
              label="Conclusão"
              valor={
                inventario.dataConclusao
                  ? new Date(inventario.dataConclusao).toLocaleString("pt-BR")
                  : null
              }
            />
            <Campo
              label="Aberto por"
              valor={inventario.usuarioAbertura?.nome}
            />
            <Campo
              label="Concluído por"
              valor={inventario.usuarioConclusao?.nome}
            />
            <Campo label="Descrição" valor={inventario.descricao} />
            <Campo label="Observação" valor={inventario.observacao} />
          </div>
        </CrudCard>
        <CrudCard>
          <h2 className="text-lg font-semibold">Itens do inventário</h2>
            ...
            <Table>
              ...
              <TableBody>
                {itens.map((item) => (
                  <TableRow key={item.id}>
                    ...
                    <TableCell className="text-right">
                      {formatarQuantidade(item.quantidadeSistema)}
                    </TableCell>
                    <TableCell className="text-right">
                      {formatarQuantidade(item.quantidadeContada)}
                    </TableCell>
                    <TableCell className="text-right">
                    ...
                    <TableCell>
                      {item.status === "CONTADO" ? "Contado" : "Pendente"}
                    </TableCell>
                    <TableCell>
                      <ContarItemInventarioModal
                ...
                {!itens.length && (
                  <TableRow>
                    <TableCell colSpan={8}>
                      <CrudEmpty message="Este inventário não possui itens." />
                    </TableCell>
                  </TableRow>
                )}
              </TableBody>
